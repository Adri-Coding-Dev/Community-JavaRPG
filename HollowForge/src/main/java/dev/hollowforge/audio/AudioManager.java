...
public class AudioManager {
    ...
    public boolean playMusic(String musicPath, int fadeInMs) {
        ...
        //Intentar aplicar la cancion al menu principal
        try {
    ...
    /**
     * Cambia el volumen de la música actual en tiempo real.
     * @param volume valor entre 0 y 1
     */
    public void setVolume(double volume) {
        this.targetVolume = Math.max(0, Math.min(1, volume));
        if (currentPlayer != null) {
            // Aplicar inmediatamente, incluso si hay un fade en curso
            currentPlayer.setVolume(targetVolume);
            // Opcional: si hay un fade, detenerlo para que no sobreescriba
            if (fadeTimer != null) {
                fadeTimer.stop();
                fadeTimer = null;
            }
